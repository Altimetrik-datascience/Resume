...
Primary_Skills_data_fin <- data.frame(do.call('rbind', strsplit(as.character(Primary_Skills_data),',',fixed=TRUE)))
Secondary_Skills_data_fin <- data.frame(do.call('rbind', strsplit(as.character(Secondary_Skills_data),',',fixed=TRUE)))
...
for(i in 1:length(temp)){
  ...
  Resume_output[i,(no_col+2)] <- spl[[i]][which(grepl("Date of Birth",spl[[i]]))+1]
  Resume_output[i,(no_col+3)] <- spl[[i]][which(grepl("Permanent Address",spl[[i]]))+1]
  Resume_output[i,(no_col+4)] <- spl[[i]][which(grepl("Contact Numbers",spl[[i]]))+1]
...
for(p in 1:ncol(Language_known)){
  colnames(Language_known)[p] <- paste0("Language",p)
  #colnames(Secondary_Skills_data_fin)[p] <- paste0("Secondary_Skill",p)
}
...
write.csv(Resume_output1,file = "Resume_feature_sample.csv")
